...
# 之前没做过地图的算法，这题没有头绪写，后续复习看吧，为了拿积分也是不择手段
class Solution:
    def minimalSteps(self, maze: List[str]) -> int:
        # 1. 状态压缩动态规划

        # 2. BFS+DP+状态压缩
        # https://leetcode-cn.com/problems/xun-bao/solution/python3-bfsdpzhuang-tai-ya-suo-by-li-bo-8/
        # 四个方向
        dd = [(-1, 0), (0, 1), (1, 0), (0, -1)]
        ...
        return ans
        # 执行用时：3440 ms, 在所有 Python3 提交中击败了53.66 % 的用户
        # 内存消耗：43.4 MB, 在所有 Python3 提交中击败了63.42 % 的用户
...
if __name__ == '__main__':
    ...
    for test_index, test_case in enumerate(test_list, start=1):
        *test, result = test_case
        test_result = s.minimalSteps(*test)
